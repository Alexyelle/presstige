...
function presstige_get_settings() {
	...
	$output['presstige_page_title'] 		= __('Theme Options','presstige');
...
function  presstige_section_fn($desc) {
	// echo "<p>" . __('Settings for this section','presstige') . "</p>";
}
	...
	switch ( $type ) {
		...
		case "multi-text":
			foreach($choices as $item) {
				...
				$item[0] = esc_html__($item[0], 'presstige');
				if (!empty($options[$id])) {
		...
		case 'select':
			echo "<select id='$id' class='select$field_class' name='" . $presstige_option_name . "[$id]'>";
				foreach($choices as $item) {
					$value 	= esc_attr($item, 'presstige');
					$item 	= esc_html($item, 'presstige');
		...
		case 'select2':
			...
			foreach($choices as $item) {
				...
				$item = explode("|",$item);
				$item[0] = esc_html($item[0], 'presstige');
...
?>
	...
	<div class="wrap">
		...
		<form action="options.php" enctype="multipart/form-data" method="post">
			...
			?>
			<p class="submit">
				<input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Save Changes','presstige'); ?>" />
			</p>
					...
					switch ( $option['class'] ) {
						...
						case 'numeric':
							...
							if(is_numeric($input[$option['id']]) == FALSE) {
								add_settings_error(
									...
									__('Expecting a Numeric value! Please fix.','presstige'), // error message
						...
						case 'multinumeric':
							...
							if($input[$option['id']] !=''){
								...
								$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : __('Expecting comma separated numeric values','presstige');
							}else{
							...
							if($input[$option['id']] !='' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
								add_settings_error(
									...
									__('Expecting comma separated numeric values! Please fix.','presstige'), // error message
									'error' // type of message
								);
							}
						break;
						...
						case 'email':
							...
							$input[$option['id']] 		= trim($input[$option['id']]); // trim whitespace
							if($input[$option['id']] != ''){
								$valid_input[$option['id']] = (is_email($input[$option['id']])!== FALSE) ? $input[$option['id']] : __('Invalid email! Please re-enter!','presstige');
							}elseif($input[$option['id']] == ''){
								$valid_input[$option['id']] = __('This setting field cannot be empty! Please enter a valid email address.','presstige');
							}
							...
							if(is_email($input[$option['id']])== FALSE || $input[$option['id']] == '') {
								add_settings_error(
									...
									__('Please enter a valid email address.','presstige'), // error message
									'error' // type of message
								);
							}
						break;
